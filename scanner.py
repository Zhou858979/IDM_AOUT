...
class Scanner:
    def __init__(self, config):
        ...
        if self.sensor is not None:
            self.sensor = self.sensor.lower()
        if self.sensor_alt is not None:
            self.sensor_alt = self.sensor_alt.lower()
        ...
        for sensor in [self.sensor, self.sensor_alt]:
            if sensor:  # Ensure the sensor is not None
                ...
                self.gcode.register_command("_" + sensor_name + "_TEMP_COMPENSATE_APPLY",
                                            self.cmd_SCANNER_TEMP_COMPENSATE_APPLY,
                                            desc=self.cmd_SCANNER_TEMP_COMPENSATE_APPLY_help)
                self.gcode.register_command("_" + sensor_name + "_WAIT_COIL_TEMP",
                                            self.cmd_SCANNER_WAIT_COIL_TEMP,
                                            desc=self.cmd_SCANNER_WAIT_COIL_TEMP_help)
    ...
    def _get_temp_data_dir(self):
        path = os.path.expanduser("~/IDM")
        if not os.path.exists(path):
            os.makedirs(path)
        return path

    def _resolve_stream_filename(self, fn):
        fn = os.path.expanduser(fn)
        if os.path.isabs(fn):
            return fn
        return os.path.join(self._get_temp_data_dir(), fn)
    ...
    def cmd_SCANNER_STREAM(self, gcmd):
        if self._log_stream is not None:
            ...
            f = None
            completion_cb = None
            fn = self._resolve_stream_filename(gcmd.get("FILENAME"))
            f = open(fn, "w")
            def close_file():
                f.close()
    ...
    cmd_SCANNER_WAIT_COIL_TEMP_help = "Wait for scanner coil temperature"
    def cmd_SCANNER_WAIT_COIL_TEMP(self, gcmd):
        min_temp = gcmd.get_float("MINIMUM", None)
        max_temp = gcmd.get_float("MAXIMUM", None)
        if min_temp is None and max_temp is None:
            raise gcmd.error("Must provide MINIMUM and/or MAXIMUM")
        reactor = self.printer.get_reactor()
        eventtime = reactor.monotonic()
        while True:
            temp = self.last_temp
            if (min_temp is None or temp >= min_temp) and \
               (max_temp is None or temp <= max_temp):
                break
            eventtime = reactor.pause(eventtime + 1.0)
        gcmd.respond_info("Scanner coil temperature %.2fC reached target" % (self.last_temp,))

    cmd_SCANNER_TEMP_COMPENSATE_APPLY_help = "Fit and save scanner temperature compensation"
    def cmd_SCANNER_TEMP_COMPENSATE_APPLY(self, gcmd):
        try:
            from . import arg_fit
        except Exception as e:
            logging.exception("Scanner temp compensation module import failed")
            raise gcmd.error(
                "Unable to import arg_fit.py: %s. Re-run install.sh and make "
                "sure scipy/matplotlib are installed in klippy-env." % (str(e),))

        base_dir = self._get_temp_data_dir()
        paths = [os.path.join(base_dir, "data%d" % (i,)) for i in range(1, 4)]
        missing = [path for path in paths if not os.path.exists(path)]
        if missing:
            raise gcmd.error("Missing scanner temperature data: %s" % (", ".join(missing),))

        fmin = self.fmin
        if fmin is None:
            fmin = arg_fit.DEFAULT_FMIN
            gcmd.respond_info("Scanner fmin is not available, using default %.5f" % (fmin,))
        fmin_temp = self.model_temp.fmin_temp if self.model_temp else arg_fit.DEFAULT_FMIN_TEMP
        output_png = os.path.join(base_dir, "fit_output.png")

        try:
            result = arg_fit.fit_from_files(paths, output_png, fmin, fmin_temp)
        except Exception as e:
            logging.exception("Scanner temp compensation fit failed")
            raise gcmd.error("Scanner temperature compensation fit failed: %s" % (str(e),))

        configfile = self.printer.lookup_object("configfile")
        configfile.set("scanner", "tc_a_a", str(result["tc_a_a"]))
        configfile.set("scanner", "tc_a_b", str(result["tc_a_b"]))
        configfile.set("scanner", "tc_b_a", str(result["tc_b_a"]))
        configfile.set("scanner", "tc_b_b", str(result["tc_b_b"]))

        self.model_temp_builder.parameters["a_a"] = result["tc_a_a"]
        self.model_temp_builder.parameters["a_b"] = result["tc_a_b"]
        self.model_temp_builder.parameters["b_a"] = result["tc_b_a"]
        self.model_temp_builder.parameters["b_b"] = result["tc_b_b"]
        self.model_temp_builder.parameters["fmin"] = fmin
        self.model_temp_builder.parameters["fmin_temp"] = fmin_temp
        self.model_temp = self.model_temp_builder.build()
        if self.model_temp:
            self.fmin = self.model_temp.fmin

        gcmd.respond_info("Scanner temperature compensation fit complete")
        gcmd.respond_info("tc_a_a: %s" % (result["tc_a_a"],))
        gcmd.respond_info("tc_a_b: %s" % (result["tc_a_b"],))
        gcmd.respond_info("tc_b_a: %s" % (result["tc_b_a"],))
        gcmd.respond_info("tc_b_b: %s" % (result["tc_b_b"],))
        gcmd.respond_info("Fit image saved to %s" % (output_png,))
        gcmd.respond_info("Check the fit image, then run SAVE_CONFIG to persist these values")
...
def register_scanner_temp_sensor(config, scanner, name, temp):
    config.get_printer().add_object(name, temp)
    pheaters = scanner.printer.load_object(config, "heaters")
    if name not in pheaters.available_sensors:
        pheaters.available_sensors.append(name)
    for attr in ("sensor_devices", "sensors"):
        devices = getattr(pheaters, attr, None)
...
def load_config(config):
    ...
    if scanner.sensor == "cartographer":
        register_scanner_temp_sensor(config, scanner, "temperature_sensor cartographer_coil", temp)
    elif scanner.sensor == "idm":
        register_scanner_temp_sensor(config, scanner, "temperature_sensor idm_coil", temp)
    elif scanner.sensor == "eddy":
        register_scanner_temp_sensor(config, scanner, "temperature_sensor eddy_coil", temp)
    else:
        register_scanner_temp_sensor(config, scanner, "temperature_sensor scanner_coil", temp)
    return scanner
